...
template <typename Sig, typename F>
memoize_helper<Sig, std::decay_t<F>> memoize_helper_r(F &&f) {
    return { std::forward<F>(f), null_param {} };
}

template <typename F>
unsigned int fib(F &&fibmemo, unsigned int n) {
    return n == 0 ? 0 : n == 1 ? 1 : fibmemo(n - 1) + fibmemo(n - 2);
}

int main(int argc, char *argv[]) {

    auto fibmemo = memoize_helper_r<unsigned int(unsigned int)>([](auto &fib, unsigned int n) {
        std::cout << "Calulating " << n << "!\n";
        return n == 0 ? 0 : n == 1 ? 1 : fib(n - 1) + fib(n - 2);
    });

    std::cout << fibmemo(12) << std::endl;
    std::cout << std::endl;
    std::cout << fibmemo(12) << std::endl;
